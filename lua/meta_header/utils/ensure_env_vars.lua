...
--    Updated : 2025/02/14 18:48:26 by aallali                  --
-- ************************************************************ --
...
local function ensure_env_vars()
	...
	if
		not os.getenv("HDR_AUTHOR")
		or not os.getenv("HDR_USERNAME")
		or not os.getenv("HDR_PROJECT")
		or not os.getenv("HDR_LICENSE")
	then
...
echo "LICENSE: $HDR_LICENSE"
			...
			-- Notify the user
			vim.notify(
				"HDR environment variables are not set. Template created at "
					.. hdr_meta_path
					.. ". Please edit and source it.",
				vim.log.levels.INFO
			)
		else
			-- Notify the user if the template already exists
			vim.notify(
				"HDR environment variables are not set. Please source " .. hdr_meta_path .. ".",
				vim.log.levels.WARN
			)
		end
	end
